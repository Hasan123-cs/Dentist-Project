...
export default function TreatmentStats() {
    ...
    const loadTreatments = async () => {
      try {
        ...
        const revenueResponse = await fetch(
          "https://localhost:7166/api/patients/dashboard-revenue",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const stats = [
    ...
    {
      title: "Completed",
      value: loading ? "..." : completedTreatments,
      icon: <CheckCircle />,
      color: "#16a34a",
    },
    ...
    {
      title: "Pending",
      value: loading ? "..." : pendingTreatments,
      icon: <Pending />,
      color: "#f59e0b",
    },
  ];
        ...
        <Grid item xs={12} sm={6} lg={3} key={item.title}>
          ...
          >
            ...
            <Box>
              ...
              <Typography fontSize={28} fontWeight={800} color="#092c57">
                {item.value}
              </Typography>
            </Box>
          </Paper>
